...
const NewsCategories = (categories) => {
    categories.forEach(category => {
        const categoriesField = document.getElementById('section');
        const url = `https://openapi.programming-hero.com/api/news/category/${category.category_id}`;
        const btnRes = document.createElement('button');
        btnRes.classList.add('btn', 'btn-light', 'text-center');
        btnRes.innerHTML = `
        <p class="mb-0" onclick="loadNewsAll('${url}','${category.category_name}')">${category.category_name}</p>
        `;
        categoriesField.appendChild(btnRes);
    })
}

const loadNewsAll = (ulrTwo,categoryName) => {
    toggleSpinner(true);
    fetch(ulrTwo)
        .then(res => res.json())
    ...
    if(newsList.length > 0){
        newsList.forEach(news => {
            ...
            if(fullNews.length > 350){
                newsDetail = fullNews.slice(0, 200) + "&hellip;";
            }
            else{
                newsDetail = fullNews;
            }
            ...
            div.innerHTML = `
                <div class="card mb-3 p-3 border border-0 shadow-lg">
                    <div class="row g-0">
        ...
        div.innerHTML=`
        <h4> We do not found any news here.</h4>
        `;
        newsSection.appendChild(div);
        toggleSpinner(false);
    }
